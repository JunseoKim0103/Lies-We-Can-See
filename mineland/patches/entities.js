...
function getEntityMesh (entity, scene) {
  if (entity.name) {
    try {
      const e = new Entity('1.16.4', entity.name, scene, entity.username, entity.equipment)
...
class Entities {
  ...
  update (entity) {
    ...
    if (entity.equipmentChanged && this.entities[entity.id]) {
      const oldMesh = this.entities[entity.id]
      this.scene.remove(oldMesh)
      dispose3(oldMesh)
      const newMesh = getEntityMesh(entity, this.scene)
      if (newMesh) {
        newMesh.position.copy(oldMesh.position)
        newMesh.rotation.copy(oldMesh.rotation)
        this.entities[entity.id] = newMesh
