...
new GoalNear(x, 65, z, range); // Move the bot to a block within the specified range of the specified block. Always use Y=65. `x`, `z`, and `range` are `number`
new GoalXZ(x, z); // Useful for long-range goals that don't have a specific Y level. `x` and `z` are `number`
new GoalGetToBlock(x, 65, z); // Not get into the block, but get directly adjacent to it. Always use Y=65. Useful for fishing, farming, filling bucket, and beds. `x` and `z` are `number`
