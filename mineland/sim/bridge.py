...
class Bridge:
    ...
    def addOverheadCamera(self, camera_id, bot_name=None, width=512, height=512):
        self.camera_set.add(camera_id)
        res = requests.post(
            f"{self.mineflayer_host_port}/addOverheadCamera",
            json={
                "camera_id": camera_id,
                "bot_name": bot_name,
                "width": width,
        ...
        if res.status_code != 200:
            raise RuntimeError(f"[Bridge] Failed to add overhead camera: {res.text}")
        return res.json()

    def enableSelfMeshOnCamera(self, camera_id):
        res = requests.post(
            f"{self.mineflayer_host_port}/enableSelfMeshOnCamera",
            json={"camera_id": camera_id},
            timeout=self.request_timeout,
        )
        if res.status_code != 200:
            raise RuntimeError(f"[Bridge] Failed to enable self mesh: {res.text}")
        return res.json()
