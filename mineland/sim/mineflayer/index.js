...
const PORT = parseInt(process.env.MINEFLAYER_PORT, 10) || 21301;
const app = express();
...
app.post("/step_lst", (req, res) => {
    ...
    setTimeout(()=>{
        ...
        const debugMessages = bot_manager.getDebugMessages();
        bot_manager.clearDebugMessages();
...
app.post("/addOverheadCamera", (req, res) => {
    const { bot_name, camera_id, width, height } = req.body;
    if (!camera_id) {
        return res.status(400).json({return_code: 400, error: 'Missing camera_id'});
    }
    try {
        bot_manager.addOverheadCamera(
            bot_name || null,
            camera_id,
            width || 512,
            height || 512
        ...
        res.status(500).json({return_code: 500, error: e.message})
    }
})

app.post("/enableSelfMeshOnCamera", (req, res) => {
    const { camera_id } = req.body;
    if (!camera_id) {
        return res.status(400).json({return_code: 400, error: 'Missing camera_id'});
    }
    try {
        bot_manager.enableSelfMeshOnCamera(camera_id);
        res.status(200).json({return_code: 200})
    } catch (e) {
        res.status(500).json({return_code: 500, error: e.message})
    }
