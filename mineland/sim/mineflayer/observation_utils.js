...
class ObservationUtils {
    ...
    static getObservation(bot, viewer_manager, tick) {
        ...
        return {
            ...
            // ===== Tags =====
            tags: bot.entity && bot.entity.metadata && bot.entity.metadata[14] 
                ? (Array.isArray(bot.entity.metadata[14]) ? bot.entity.metadata[14] : [])
                : [],

            // ===== Scoreboard =====
            scoreboard: {
                mission1: bot.mission1 !== undefined ? bot.mission1 : null,
                mission2: bot.mission2 !== undefined ? bot.mission2 : null,
                mission3: bot.mission3 !== undefined ? bot.mission3 : null,
                mission4: bot.mission4 !== undefined ? bot.mission4 : null,
                mission5: bot.mission5 !== undefined ? bot.mission5 : null,
                mission6: bot.mission6 !== undefined ? bot.mission6 : null,
                mission7: bot.mission7 !== undefined ? bot.mission7 : null,
                mission8: bot.mission8 !== undefined ? bot.mission8 : null,
                mission9: bot.mission9 !== undefined ? bot.mission9 : null,
                mission10: bot.mission10 !== undefined ? bot.mission10 : null,
                mission11: bot.mission11 !== undefined ? bot.mission11 : null,
                mission12: bot.mission12 !== undefined ? bot.mission12 : null,
                mission13: bot.mission13 !== undefined ? bot.mission13 : null,
                mission14: bot.mission14 !== undefined ? bot.mission14 : null,
                mission15: bot.mission15 !== undefined ? bot.mission15 : null,
                mission16: bot.mission16 !== undefined ? bot.mission16 : null,
                mission17: bot.mission17 !== undefined ? bot.mission17 : null,
                mission18: bot.mission18 !== undefined ? bot.mission18 : null,
                mission19: bot.mission19 !== undefined ? bot.mission19 : null,
                mission20: bot.mission20 !== undefined ? bot.mission20 : null,
                atk_ready: bot.atk_ready !== undefined ? bot.atk_ready : null,
                phase: bot.phase !== undefined ? bot.phase : null,
                meeting_1min: bot.meeting_1min !== undefined ? bot.meeting_1min : null,
                game_end: bot.game_end !== undefined ? bot.game_end : null,
                ghost: bot.ghost !== undefined ? bot.ghost : null,
                talk: bot.talk !== undefined ? bot.talk : null,
                area: bot.area !== undefined ? bot.area : null,
                doing_mission: bot.doing_mission !== undefined ? bot.doing_mission : null,
                reported_id: bot.reported_id !== undefined ? bot.reported_id : null,
                scoreboard_by_entity: bot.scoreboard || {},
            },
    ...
    static getTargetEntities(bot) {
        ...
        // Precompute viewer state for in_sight check.
        // in_sight = (yaw frustum ±60°) AND (LOS raycast: no block between eyes).
        // EgocentricStateBuilder uses this flag to drop entities the bot
        // cannot visually see (vision-restricted ego).
        const botEye = bot.entity ? bot.entity.position.offset(0, 1.62, 0) : null;
        const botYaw = bot.entity ? bot.entity.yaw : 0;
        const FOV_HALF = Math.PI / 3; // ±60° horizontal

        const entities = Object.values(bot.entities).filter(entity => {
            if (entity.name === 'armor_stand') return true;
            if (entity.type === 'player' && entity.username && entity.username !== bot.username) return true;
            return entity.metadata && entity.metadata[2];
        }).map((entity) => {
            const {position, metadata, name} = entity;
            let entityName;
            if (entity.type === 'player') {
                entityName = entity.username;
            } else {
                entityName = name === 'armor_stand' ? 'armor_stand' : getName(metadata[2]);
            }

            // ── in_sight (LOS + frustum) ──
            let in_sight = true;
            if (botEye && position) {
                const targetEye = position.offset(0, 1.5, 0); // ~eye height for players/stands
                const dx = targetEye.x - botEye.x;
                const dy = targetEye.y - botEye.y;
                const dz = targetEye.z - botEye.z;
                const dist = Math.sqrt(dx * dx + dy * dy + dz * dz);

                // Frustum: yaw delta vs angle-to-target (ignore pitch).
                // Minecraft yaw: 0=south(+z), 90=west(-x), 180=north(-z), 270=east(+x).
                // angleToTarget in same convention = atan2(-dx, dz).
                const angleToTarget = Math.atan2(-dx, dz);
                const yawDiff = ((botYaw - angleToTarget + Math.PI) % (2 * Math.PI)) - Math.PI;
                if (Math.abs(yawDiff) > FOV_HALF) {
                    in_sight = false;
                }

                // LOS raycast (only if frustum already passes).
                if (in_sight && dist > 0.5 && bot.world && bot.world.raycast) {
                    try {
                        const dir = new Vec3(dx / dist, dy / dist, dz / dist);
                        const hit = bot.world.raycast(botEye, dir, dist - 0.4);
                        if (hit) in_sight = false;
                    } catch (e) {
                        // raycast edge cases (unloaded chunks etc.): default to visible
                    }
                }
            }
            ...
            return {
                "name": entityName,
                "kind": name,
                "health": metadata ? metadata[9] : null,
                "distance": bot.entity ? bot.entity.position.distanceTo(position) : null,
                "in_sight": in_sight,
                position,
            };
        });
