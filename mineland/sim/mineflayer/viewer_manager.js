...
class ViewerManager {
     ...
     * Create a camera
     */
    async addCamera(bot, camera_id, image_width, image_height, { showSelfMesh = false, viewDistance = 3 } = {}) {
        this.cameras[camera_id] = mineflayerFreecamera(bot, { viewDistance, height: image_height, width: image_width, showSelfMesh })
    ...
    enableSelfMeshOnCamera(camera_id) {
        if (this.cameras[camera_id] && this.cameras[camera_id].enableSelfMesh) {
            this.cameras[camera_id].enableSelfMesh()
        }
    }
