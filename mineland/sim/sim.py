...
class MineLand(gym.Env):
    def __init__(
        ...
        # ===== Instance isolation (one container, many concurrent games) =====
        # instance_id offsets every port so multiple servers can coexist.
        self.instance_id = instance_id
        self.mineflayer_port = 21301 + instance_id

        # ===== Server =====
        if server_host is None:
            self.server_host = "localhost"
            self.server_port = 25565 + instance_id
            print("Starting server...")
            self.server_manager = ServerManager(
                is_printing_server_info=is_printing_server_info,
                instance_id=instance_id,
                server_port=self.server_port,
            )
            if world_type == "normal":
                self.server_manager.select_to_normal_world()
            elif world_type == "amongus_2im":
                self.server_manager.select_to_construction_world("amongus_2im")
            else :
                self.server_manager.select_to_construction_world("amongus")
        ...
        mineflayer_manager = MineflayerManager(is_printing_mineflayer_info=is_printing_mineflayer_info, port=self.mineflayer_port)
        ...
        self.bridge = Bridge(
            ...
            mineflayer_port=self.mineflayer_port,
    ...
    def reset(self) -> List[Observation]:
        ...
        if self.server_manager is not None:
            ...
            # give op to human4
            self.server_manager.execute("function mypack:start")
            self.server_manager.execute("op human1")


        
            if self.enable_auto_pause:
                # Runtick 20 ticks (1 second) to execute all preset commands
                self.server_manager.execute("runtick 100")
